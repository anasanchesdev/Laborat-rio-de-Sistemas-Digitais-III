...
#define _100us (long int)(Fclk*100e-6)    // Delay de 100 micro segundos
#define _5ms   (long int)(Fclk*5e-3)      // Delay de 5 mili segundos
#define _2ms   (long int)(Fclk*2e-3)      // Delay de 2 mili segundos
#define _250ms (long int)(Fclk*250e-3)    // Delay de 250 mili segundos
...
/*
Função que envia uma sequencia de caracteres para o display.
*/
void EnviaString(char *string)
{
  while(*string){  // Enquanto o caracter não for NULL...
    EnviaDado(*string);    // Envia o conteúdo do ponteiro
    string++;              // Muda o ponteiro para a próxima posição
  }
}
